...
class NeuronGraph:
    ...
    def add_connection(
        ...
        a, b = concept_a.lower().strip(), concept_b.lower().strip()
        ...
        if self.G.has_edge(a, b):
            self.G.edges[a, b]["co_occurrences"] = (
                self.G.edges[a, b].get("co_occurrences", 1) + 1
            )
            self.G.edges[a, b]["weight"] = min(
                1.0, self.G.edges[a, b]["weight"] + 0.05
            )
        else:
            self.G.add_edge(
                a, b,
    ...
    def explore(self, concept: str, depth: int = 2) -> dict:
        ...
        if concept not in self.G:
            # Cherche les concepts proches par nom
        ...
        for target, path in nx.single_source_shortest_path(
            self.G, concept, cutoff=depth
        ).items():
            if target == concept:
                continue
            d = len(path) - 1
            # Cherche l'arête sur le chemin direct
            if d == 1:
        ...
        # Trier : distance puis force
        sorted_neighbors = dict(
            sorted(neighbors.items(), key=lambda x: (x[1]["distance"], -x[1]["strength"]))[:25]
        )
    ...
    def get_map(self, max_nodes: int = 80) -> dict:
        """Données pour la visualisation D3.js force-graph."""
        all_nodes = sorted(
